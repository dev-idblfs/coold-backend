            ...
            if (params.email && params.step && params.step == 1) {
                ...
                if (result.status == 200) {
                    res.send({
                        status: 200,
                        message:
                            "OTP has been sent to your Email : " + params.email,
                    });
                } else
                    res.send({
                        status: 200,
                ...
                if (
                    list.body[0].otp == params.otp &&
                    list.body[0].email == params.email
                ) {
                    let data = {
                        updatedAt: Date.now(),
                        isVerified: 1,
                    ...
                    result = await resume.update(
                        { email: list.body[0].email },
                        data
                    );
            ...
            upload(req, res, async (err) => {
                ...
                if (params.email && params.step && params.step == 3) {
                    if (["example@domain.com"].includes(params.email)) {
                        return res.send({
                            status: 200,
                            message: "Thank you For Subbmitted",
                        });
                    }
                    delete params.step;
                    filter = {
                    ...
                    let fsFile = fs.readFileSync(
                        `${ROOT_DIR}/resumes/${req.file.filename}`
                    );
    ...
    if (Object.keys(req.body).length > 0) {
        try {
            ...
            if (params.subsEmail) {
                ...
                const subcribeNewFeed = require(ROOT_DIR +
                    "/modals/subcribers_email");
                ...
                if (isfound.data.length > 0) {
                    return res.send({
                        status: 304,
                        message: "This is Email already subcribered",
                    });
                }
                ...
                if (result.status == 200) {
                    res.send({
                        status: 200,
                        message: "Thnak you for Subcribing us",
                    });
                } else
                    res.send({
                        status: 500,
